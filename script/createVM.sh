...
cp ./machineOdoo/interfaces ~/tmp/

#On les modifie afin de les insérer directement sur la machine
sed -i -e "s/$2/192.168.194.1/g" ~/tmp/interfaces


scp ./machineOdoo/environment root@$ipMachine:./etc/
scp ./machineOdoo/timesyncd.conf root@$ipMachine:./etc/systemd/
scp ~/machineOdoo/hosts root@$ipMachine:./etc/
...
scp -r ./fichierDocker root@$ipMachine
scp  ./installDocker.sh root@$ipMachine

ssh root@$2 "su -c source installDocker.sh"

#Faire des read pour demander l'adresse et le nom de la machine 